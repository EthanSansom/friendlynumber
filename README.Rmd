...
{friendlynumber} functions are intended to be used internally by other functions
(e.g. for generating friendly error messages). To this end, {friendlynumber} is
writing in base R and has no Imports.
...
* [{bignum}](https://davidchall.github.io/bignum/) `<bignum_biginteger>` vectors, which can store arbitrarily large integers
...
that {friendlynumber} function report.
...
the [{english}](https://cran.r-project.org/web/packages/english/index.html) and 
...
`*_friendly()` functions have a (very-slightly) slower `*_friendly_safe()` 
...
This package was originally inspired by the [{english}](https://cran.r-project.org/web/packages/english/index.html)
package by John Fox, Bill Venables, Anthony Damico and Anne Pier Salverda, which 
spurred my brief fixation with the programming puzzle of converting numbers to 
numerals.
...
naming arbitrarily large numbers was used for the naming of numbers greater than 10^3003
* Steve Olsen's table of ["Big-Ass Numbers"](https://www.olsenhome.com/bignumbers/) was very helpful for testing `number_friendly()`
